...
class VerifyRequestTest {
    ...
    void blankSecretThrows() {
        assertThrows(IllegalArgumentException.class, () -> new VerifyRequest().setSecret("   "));
    }
    ...
    @Test
    void inequalityOnDifferentSecret() {
        assertNotEquals(buildValid(), buildValid().secret("different=="));
    }

    @Test
    void inequalityOnDifferentClientIp() {
        assertNotEquals(buildValid(), buildValid().clientIp("9.9.9.9"));
    }

    @Test
    void inequalityOnDifferentClientToken() {
        assertNotEquals(buildValid(), buildValid().clientToken("other-token=="));
    }

    @Test
    void inequalityOnDifferentClientUserAgent() {
        assertNotEquals(buildValid(), buildValid().clientUserAgent("curl/8.0"));
    }
}
